...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help="Behavior when the default target cannot be written. Defaults to CODEX_ACTIVITY_LOG_FALLBACK or auto.",
    )
...
def resolve_fallback_mode(args: argparse.Namespace) -> str:
    ...
    env_value = os.environ.get("CODEX_ACTIVITY_LOG_FALLBACK", "").strip().lower()
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"[change-logger] default log target unavailable ({exc}); wrote to {final_path}",
            file=sys.stderr,
        )
